CREATE TABLE IF NOT EXISTS device_operations (
    ...
    operation_type TEXT NOT NULL CHECK (operation_type IN ('provision', 'deactivate', 'unprovision')),
...
CREATE TABLE IF NOT EXISTS device_message_outbox (
    ...
    message_type TEXT NOT NULL CHECK (message_type IN ('DeviceProvisionRequested', 'DeviceDeactivateRequested', 'DeviceUnprovisionRequested')),
...
CREATE TABLE IF NOT EXISTS device_message_inbox (
    ...
    message_type TEXT NOT NULL CHECK (message_type IN ('DeviceProvisionSucceeded', 'DeviceProvisionFailed', 'DeviceDeactivateSucceeded', 'DeviceDeactivateFailed', 'DeviceUnprovisionSucceeded', 'DeviceUnprovisionFailed', 'DeviceOnlineChanged', 'DeviceMetadataChanged')),
    schema_version TEXT NOT NULL CHECK (schema_version IN ('1.0')),
    partition_key TEXT NOT NULL CHECK (btrim(partition_key) <> ''),
    payload JSONB NOT NULL DEFAULT '{}'::jsonb,
