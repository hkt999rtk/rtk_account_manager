...
stored in `brand_cloud_users`. It may
represent that brand cloud's owner, admin, member, or end user. The same email
address in two different brand clouds represents two different
customer-visible accounts because each brand cloud is presented as its own
service. Brand-cloud owner/admin/member/end-user accounts are not platform
accounts, even when they use the same email address as a platform user or a
user in another brand cloud.
