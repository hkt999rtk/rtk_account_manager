...
and password. Platform users may carry global operator privileges such as
`platform_admin=true`; they are not customer-visible brand-cloud accounts.
...
A brand-cloud user is a human account scoped to exactly one brand cloud. It may
represent that brand cloud's owner, admin, member, or end user. The same email
address in two different brand clouds represents two different customer-visible
accounts because each brand cloud is presented as its own service.

Brand-cloud login scope is resolved from the brand domain or tenant slug before
email/password authentication. Authentication responses and session tokens must
...
for new brand-cloud end-user identities.
...
Brand-cloud user storage is the target model for brand-cloud owners, admins,
members, and end users. It must be introduced as dedicated brand-scoped identity
storage rather than by removing global uniqueness from `users.email`.

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `id` | UUID | Yes | Primary key, generated by server/database. |
| `brand_cloud_id` | UUID | Yes | References `organizations.id` where `organization_kind='brand_cloud'`. |
| `email` | Text | Yes | Normalized email address, unique only within `brand_cloud_id`. |
| `password_hash` | Text | Yes | Hashed password for this brand-cloud account. |
| `display_name` | Text | No | Brand-cloud scoped display name. |
| `email_verified` | Boolean | Yes | Set after brand-cloud email verification succeeds or operator provisioning marks the account verified. |
| `email_verified_at` | Timestamp | No | Time email verification completed. |
| `signup_pending_verification` | Boolean | Yes | Brand-cloud public signup accounts cannot log in until this is cleared. |
| `created_at` | Timestamp | Yes | Creation timestamp. |
| `updated_at` | Timestamp | Yes | Last update timestamp. |
| `disabled_at` | Timestamp | No | Set when this brand-cloud account is disabled. |

Target constraints:

- `(brand_cloud_id, email)` must be unique after lowercasing and trimming email.
...
- Brand-cloud `owner`, `admin`, `member`, and `end_user` roles are scoped to one
  brand cloud and belong to brand-cloud users. They must not imply
  `platform_admin`.
...
| `POST` | `/v1/admin/brand-clouds/:brandCloudId/members` | Yes | Platform admin | Assign or update a brand-cloud user's role inside that brand cloud. |
| `POST` | `/v1/admin/brand-clouds/:brandCloudId/users` | Yes | Platform admin | Create or reactivate a brand-scoped user in that brand cloud and assign a role. |
