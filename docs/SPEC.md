...
| `POST` | `/v1/admin/brand-clouds/:brandCloudId/device-bind-jobs` | Yes | Platform admin | Synchronously bulk lookup/create staging load-test devices by `video_cloud_devid`; one chunk accepts up to 5000 items and returns per-item `created`/`existing`/`failed` results. |
