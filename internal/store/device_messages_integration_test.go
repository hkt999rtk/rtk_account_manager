...
func newStoreIntegrationEnv(t *testing.T) storeIntegrationEnv {
	...
	if _, err := db.Exec(ctx, `
			TRUNCATE acl_audit_events, external_group_mappings, role_assignments, oidc_login_states, user_identities, identity_providers, device_claims, device_claim_tokens, device_message_inbox, device_message_outbox, device_operations, refresh_tokens, device_tags, device_group_members, device_groups, devices, organization_members, organizations, users
