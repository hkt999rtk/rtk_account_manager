...
import (
	...
	"rtk_account_manager/internal/billing"
	"rtk_account_manager/internal/billingdocument"
	"rtk_account_manager/internal/database"
	"rtk_account_manager/internal/payment"
	"rtk_account_manager/internal/paymentstore"
	accountstore "rtk_account_manager/internal/store"
...
func TestBillingPersistenceInvoiceLifecycle(t *testing.T) {
	...
	if _, err := db.Exec(ctx, `TRUNCATE billing_activity_events, invoice_settlement_links, billing_invoice_documents,
		...
		organizations, users RESTART IDENTITY CASCADE`); err != nil {
		t.Fatal(err)
	}

	registered, err := accountstore.New(db).Register(ctx, accountstore.RegisterInput{
		Email: "billing-store@example.com", PasswordHash: "hash", OrganizationName: "Billing Store Test",
	})
	if err != nil {
		t.Fatal(err)
	}
	account, _, err := paymentstore.New(db).EnsureCommercialAccount(ctx, registered.Organization.ID, payment.CurrencyTWD)
	if err != nil {
		t.Fatal(err)
	}
	store := New(db)
	...
	if _, created, err := store.PutUsageFact(ctx, billing.UsageFact{
		UsageID: "usage-1", OrganizationID: registered.Organization.ID, ServiceCode: "video", MetricCode: "relay_minutes",
	...
	invoice, created, err := store.PrepareInvoice(ctx, PrepareInvoiceInput{
		OrganizationID: registered.Organization.ID, AccountID: account.ID, Currency: billing.CurrencyTWD,
	...
	if err := store.PutInvoiceDocument(ctx, registered.Organization.ID, invoice.ID, metadata, data); err != nil {
		t.Fatal(err)
	}
	settled, err := store.RecordInvoiceSettlement(ctx, registered.Organization.ID, invoice.ID, "", now)
	...
	if _, _, err := store.PutUsageFact(ctx, billing.UsageFact{
		UsageID: "late-usage", OrganizationID: registered.Organization.ID, ServiceCode: "video", MetricCode: "relay_minutes",
