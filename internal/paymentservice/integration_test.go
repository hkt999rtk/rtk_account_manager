...
func createIntegrationFixture(t *testing.T, env integrationEnv, suffix string, now time.Time) integrationFixture {
	...
	registered, err := accountstore.New(env.db).Register(ctx, accountstore.RegisterInput{
		Email: fmt.Sprintf("payment-service-%s@example.com", suffix), PasswordHash: "hash",
		OrganizationName: "Payment Service " + suffix,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	methodConsent, err := env.store.CreateConsent(ctx, paymentstore.CreateConsentInput{
		...
		TextSHA256: strings.Repeat("a", 64), AcceptedActorType: "user", AcceptedActorID: registered.User.ID,
	...
	policyConsent, err := env.store.CreateConsent(ctx, paymentstore.CreateConsentInput{
		...
		TextSHA256: strings.Repeat("c", 64), AcceptedActorType: "user", AcceptedActorID: registered.User.ID,
