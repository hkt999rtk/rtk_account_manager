...
func paymentActorType(c *gin.Context) string {
	if currentSubjectType(c) == auth.SubjectTypeBrandCloudUser {
		return store.ActorTypeBrandCloudUser
	}
	return store.ActorTypeUser
}
...
func (s *Server) writePaymentAudit(c *gin.Context, eventType, subjectType, subjectID string, payload map[string]any) bool {
	...
	if err := s.store.CreateAuditEvent(c.Request.Context(), store.AuditEventInput{
		EventType: eventType, ActorUserID: actor, OrganizationID: &orgID,
		SubjectType: subjectType, SubjectID: subjectID, Payload: payload,
	}); err != nil {
