...
func createPaymentFixture(t *testing.T, env paymentIntegrationEnv, suffix string, initialBalance, threshold, topUp int64) paymentFixture {
	...
	registered, err := accountstore.New(env.db).Register(ctx, accountstore.RegisterInput{
		Email:            fmt.Sprintf("payment-%s@example.com", suffix),
		PasswordHash:     "hash",
		OrganizationName: "Payment Test " + suffix,
	})
	if err != nil {
		t.Fatal(err)
	}
	account, created, err := env.store.EnsureCommercialAccount(ctx, registered.Organization.ID, payment.CurrencyTWD)
	if err != nil {
		t.Fatal(err)
	}
	if !created {
	...
	methodConsent, err := env.store.CreateConsent(ctx, CreateConsentInput{
		...
		AcceptedActorID:   registered.User.ID,
...
func TestLedgerWorksWithoutAutoTopUpPolicy(t *testing.T) {
	...
	ctx := context.Background()
	registered, err := accountstore.New(env.db).Register(ctx, accountstore.RegisterInput{
		Email: "payment-no-policy@example.com", PasswordHash: "hash", OrganizationName: "No Policy",
	})
	if err != nil {
		t.Fatal(err)
	}
	account, _, err := env.store.EnsureCommercialAccount(ctx, registered.Organization.ID, payment.CurrencyTWD)
	if err != nil {
		t.Fatal(err)
	}
